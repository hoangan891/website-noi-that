...
const validOrderStatuses = ['pending', 'preparing', 'shipping', 'delivered'];

// @desc    Get orders, optionally filtered by status
...
const getOrderCounts = asyncHandler(async (req, res) => {
  try {
    ...
    for (const status of validOrderStatuses) {
      counts[`${status}Orders`] = await Order.countDocuments({ status });
    }
    res.json(counts);
  } catch (error) {
...
export { createOrder, getOrder, getOrders, getPendingOrders, getOrderCounts, updateOrderStatus, clearCart, getUserOrders };