...
// @desc    Get product inventory counts (out of stock / low stock)
...
const getProductCounts = asyncHandler(async (req, res) => {
    try {
        ...
        const lowStockCount = await Product.countDocuments({ countInStock: { $gt: 0, $lte: 5 } });
        res.json({ outOfStockCount, lowStockCount });
    } catch (error) {
